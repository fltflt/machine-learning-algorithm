...
def loadDataSet():
    ...
    labelMat = []
    # 打开文件
    fr = open('D:\\机器学习实战\\机器学习实战源代码\\machinelearninginaction\\Ch05\\testSet.txt')
    # 逐行读取
    for line in fr.readlines():
        # 去掉每行两边的空白字符，并以空格分隔每行数据元素,每行数据格式-0.017612\t14.053064\t0\n，字符之间以空格\t制表符
...
def plotBestFit(weights):
    ...
    # 数据个数
    # 例如建立一个4*2的矩阵c，c.shape[1]为第二维的长度2， c.shape[0]为第一维的长度4
...
"""
函数说明：梯度上升法，一次使用全部数据集更新参数吗，并且学习率不变
...
"""
函数说明：改进的随机梯度上升法，一次随机选择一个样本更新参数,并且每次都降低alpha的大小
